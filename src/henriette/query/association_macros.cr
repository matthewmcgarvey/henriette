    ...
    macro generate_has_many_preload(model, association, query)
      ...
      def preload_{{ association[:assoc_name].id }}(preload_query : {{ query.id }})
        add_preload do |records|
          ids = records.map(&.id)
          if ids.empty?
            {{ association[:assoc_name].id }} = {} of {{ model }}::PrimaryKeyType => Array({{ model }})
          else
          ...
          records.each do |record|
            record._preloaded_{{ association[:assoc_name].id }} = {{ association[:assoc_name].id }}[record.id]? || [] of {{ model }}
          end
        end
        self
      end
